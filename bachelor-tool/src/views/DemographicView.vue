...
const educationOptions = [
  ...
  { value: 'schule', label: 'Schulabschluss (z. B. Haupt-/Real-/Mittelschule)' },
  { value: 'abitur', label: '(Fach-)Abitur' },
  { value: 'ausbildung', label: 'Berufsausbildung' },
  { value: 'hochschule', label: 'Hochschulabschluss (Bachelor/Master/Diplom etc.)' },
...
<template>
  <main>
    ...
    <form class="dfb" @submit.prevent="continue_to_part_one" @reset="handleReset" novalidate>
      ...
      <div class="dfb_field">
        <label for="competency">Kompetenzeinschätzung (digitale Geräte/Software)</label>
        <select id="competency" v-model="form.competency">
          <option value="">— Bitte wählen —</option>
          <option v-for="opt in competencyOptions" :key="opt.value" :value="opt.value">
            {{ opt.label }}
          </option>
        </select>
      </div>
      ...
      <p>Vielen Dank für das Beantworten der Fragen. Wenn Sie bereit sind können Sie mit einen
        Klick auf den Weiter-Knopf zum ersten Teil der Studie gelangen.</p>
