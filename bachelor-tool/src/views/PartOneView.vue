...
import CenteredImageTextarea from "../components/PartOnePage.vue";
...
async function handleSubmit() {
  ...
  await axios.get(
    `${store.apiUrl}/send_part_one_answer` + user_path + image_path + user_answer,
    {
      responseType: "json",
    }
  );
...
<template>
  ...
  <div class="explanation-container" v-else>
    <h2>Teil 1</h2>
    <p class="explanation">Im folgenden Teil der Studie wird Ihnen fünf Mal ein Bild gezeigt, welches ein Video zusammenfassen soll.
      Ihre Aufgabe ist es Ihre Vermutung zu beschreiben, was in dem Video passiert. </p>
